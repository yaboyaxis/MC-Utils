import { Message, BitFieldResolvable, PermissionString, GuildMember } from "discord.js";
...
export class Automod {
    ...
    async warnUser(client: AkairoClient, message: Message, member: GuildMember, spamMessages: any[]) {
        await autoModWarn(member, member.guild, "Violating Automod", message, client);
    }

    async processMsg(client: AkairoClient, message: Message) {
        if (!message.guild || message.author.id === message.client.user.id || (message.guild.ownerID === message.author.id) || (this.options.ignoreBots && message.author.bot)) {
            return false;
        }

        const isMemberIgnored = this.options.ignoredMembers.includes(message.author.id);
        if (isMemberIgnored) return false;

        const isChannelIgnored = this.options.ignoredChannels.includes(message.channel.id);
        if (isChannelIgnored) return false;

        const member = message.member || await message.guild.members.fetch(message.author);

        const memberHasIgnoredRoles = this.options.ignoredRoles.some((r: string) => member.roles.cache.has(r));
        if (memberHasIgnoredRoles) return false;

        if (this.options.ignoredPermissions.some((permission) => member.hasPermission(<BitFieldResolvable<PermissionString>>permission))) return false;
        ...
        const duplicateMatches = cachedMessages.filter((m) => m.content === message.content && (m.sentTimestamp > (currentMessage.sentTimestamp - this.options.maxDupInt)));

        if (message.content.match(urlRegexSafe({ strict: true }))) {
            if (message.deletable) await message.delete({ reason: "[AutoMod] Triggered link filter" });
            try {
                await autoModWarn(message.member, message.guild, "Sending Links", message, client);
